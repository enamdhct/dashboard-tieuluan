...
import { getAllOrder,getAll } from '@/services/refundService'
...
export default function page() {
  ...
  async function getData(){
    ...
    setArrOrder(data.requests)
    console.log(data);
    setTotalPages(data.pagination.totalPages);
    setCurrentPage(data.pagination.currentPage);
    const users = []
    for(let i = 0; i<data.requests.length;i++){
        let user = await getUser(data.requests[i].user_id)
        users.push(user)
    }
    setArrUser(users)
  }
      ...
      <table className="w-full border-collapse font-arial mt-4">
            <thead className='bg-slate-100'>
              <tr>
                ...
                <th className="border text-left p-4">Ngày yêu cầu</th>
                <th className="border text-left p-4">Người đặt</th>
                <th className="border text-left p-4">Lời nhắn</th>
                <th className="border text-center p-4">Tổng tiền</th>
                ...
                arrOrder && arrOrder.filter((item) => item.state != 'Nháp').map((item, index)=>{
                  ...
                  return(
                    <tr key={index}>
                      ...
                      <td className="border text-left p-4">{moment(item.request_date).format("DD/MM/YYYY HH:mm:ss")}</td>
                      <td className="border text-left p-4">{user && user.name}</td>
                      <td className="border text-left p-4">{item.reason && item.reason}</td>
                      <td className="border text-center p-4">{item.refund_amount.toLocaleString('vi-VN', {style: 'currency',currency: 'VND'})}</td>
                      {/* <td className="border text-center p-4">{item.paymentMethod === "COD" ? 'COD' : 'Chuyển khoản'}</td> */}
                      <td className="border text-center p-4">{
                        <span className={getStateClass(item.status)}>{item.status}</span>
                      }</td>

                      <td className="border text-left p-4">
                        <div className='flex flex-row gap-3 justify-center'>
                        {item.state === "Refund:Chờ duyệt" &&
                          <ToolTips 
                          onClick={()=>handleApprove(item._id)}
                          icon={<Icon name={'check'} className={'text-green-600 w-6 h-6'}></Icon>}
                          tooltipText={"Xác nhận đơn hàng"}
                        />}
                          <Link href={`refund/${item._id}`}>
                            <ToolTips
